...
def sys_normalized_adjacency(adj, add_self_loop=True):
   adj = sp.coo_matrix(adj)
   if add_self_loop: adj = adj + sp.eye(adj.shape[0])
...
def load_ppi():
    ...
    #train_ids = np.array([id_map[n] for n in G.nodes() if not G.node[n]['val'] and not G.node[n]['test']])  #ORIGINAL depricated
    train_ids = np.array([id_map[n] for n in G.nodes() if not G.nodes[n]['val'] and not G.nodes[n]['test']]) 
    #GAYAN   If you are using NetworkX 2.4, use G.nodes[] instead of G.node[]
