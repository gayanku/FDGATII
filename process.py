...
def full_load_data(dataset_name, splits_file_path=None, include_adj_without_self=False):
    ...
    import copy
    g_noself = copy.deepcopy(g)
    g_noself = sys_normalized_adjacency(g_noself, add_self_loop=False )
    g_noself = sparse_mx_to_torch_sparse_tensor(g_noself)

    g = sys_normalized_adjacency(g)
    g = sparse_mx_to_torch_sparse_tensor(g)

    if include_adj_without_self:
      return g,g_noself, features, labels, train_mask, val_mask, test_mask, num_features, num_labels  
